...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* El nuevo SVG es horizontal (812×312), lo dejamos ocupar más espacio */
...
// ===== FECHA: sincroniza picker nativo → display DD/MM/AAAA =====
function sincronizarFecha() {
  const raw = document.getElementById('fechaRaw').value; // YYYY-MM-DD
  if (!raw) return;
...
// ===== GENERAR PDF SIN GUARDAR (devuelve blob URL) — compartido entre WA y Gmail =====
async function generarPDFBlob() {
  ...
  // Guardar el PDF en disco (descarga automática)
  pdf.save(filename);

  return filename;
}

// ===== WHATSAPP CLIENTE (descarga PDF y luego abre WA) =====
...
// ===== GMAIL CLIENTE (descarga PDF y luego abre Gmail) =====
